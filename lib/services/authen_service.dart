...
class AuthenticationService {
    ...
    return Utils.withConnection(
      ...
      () async {
        ...
        var request = http.Request(
          'POST',
          Uri.parse('$devServer/api/v1/auth/sign-in'),
        );
  ...
  static Future<bool?> signUp({
    ...
    return await Utils.withConnection(
      ...
      () async {
        try {
          var headers = {'x-jarvis-guid': '', 'Content-Type': 'application/json'};
          var request = http.Request('POST', Uri.parse('$devServer/api/v1/auth/sign-up'));
          request.body = json.encode({"email": email, "password": password, "username": username});
          request.headers.addAll(headers);

          http.StreamedResponse response = await request.send();
          if (response.statusCode == 200 || response.statusCode == 201) {
            log('Creating New LoopyBot Account Successfully');
            return true;
          } else {
            log('Failed when Creating New LoopyBot Account: ${response.reasonPhrase}');
            return false;
          }
        } catch (err) {
  ...
  static Future<String?> getAccessToken(BuildContext context) async {
    return await Utils.withConnection(
      ...
      () async {
        ...
        final refreshToken = Provider.of<AuthenticationProvider>(context, listen: false).refreshToken;
        ...
        if (statusCode == 200 || statusCode == 201) {
          ...
          log('Jarvis Access Token=$_cachedAccessToken');
        }
  ...
  static Future<UserModel?> getCurrentUser({
    ...
    if (response.statusCode == 200 || response.statusCode == 201) {
      ...
      throw Exception('Error when getting current user: ${response.reasonPhrase}');
    }
  }
